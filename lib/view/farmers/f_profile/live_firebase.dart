...
class _LiveFirebaseDataState extends State<LiveFirebaseData> {
  ...
  void initState() {
    super.initState();
    Timer.periodic(
        Duration(seconds: 100), (Timer t) => controller.refreshApi());
  }
      ...
      if (controller.liveData.value != null) {
        return Column(children: <Widget>[
          Column(
            children: <Widget>[
              _buildDetailItem(
                  'Humidity', controller.liveData.value!.humidity.toString()),
              _buildDetailItem('Soil Temperature',
                  controller.liveData.value!.soilMoisture.toString()),
              _buildDetailItem(
                  'Temperature', controller.liveData.value!.temp.toString()),
            ],
          )
        ]);
